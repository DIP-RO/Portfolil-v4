...
const Certification = () => {
    ...
    return (
        <div ref={newsectionRef}>
            <div id='certification' className='px-6 lg:px-32 pt-32 pb-20'>
                <h2 className='head-text'>Certification</h2>
                <p className='subtitle'>Here are some of my certificate. Although I completed many <br />  but these are complete recently.</p>
                <div className='mt-12 grid gap-6 lg:gap-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
                    {
                        CertificationData.slice(0, Limit).map((data, i) => <ProjectCard animate={animate} i={i} key={i} data={data} />)
                    }
                </div>
                {
                    CertificationData.length > Limit &&
