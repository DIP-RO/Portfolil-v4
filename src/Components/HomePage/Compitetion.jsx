import { CompitationData } from '@/Constants/Constants';
...
const Compitetion = () => {
    ...
    return (
        <div ref={newsectionRef}>
            <div id='Achievement' className='px-6 lg:px-32 pt-32 pb-20'>
                <h2 className='head-text'>Achievement</h2>
                <p className='subtitle'>Here are some of my Achievement. Although I have more <br />  but these are completely change my life.</p>
                <div className='mt-12 grid gap-6 lg:gap-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
                    {
                        CompitationData.slice(0, Limit).map((data, i) => <ProjectCard animate={animate} i={i} key={i} data={data} />)
                    }
                </div>
                {
                    CompitationData.length > Limit &&
