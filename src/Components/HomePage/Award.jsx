import { AwardData } from '@/Constants/Constants';
...
const Award = () => {
    ...
    return (
        <div ref={newsectionRef}>
            <div id='award' className='px-6 lg:px-32 pt-32 pb-20'>
                <h2 className='head-text'>Award</h2>
                <p className='subtitle'>Here are some of my Award..</p>
                <div className='mt-12 grid gap-6 lg:gap-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
                    {
                        AwardData.slice(0, Limit).map((data, i) => <ProjectCard animate={animate} i={i} key={i} data={data} />)
                    }
                </div>
                {
                    AwardData.length > Limit &&
